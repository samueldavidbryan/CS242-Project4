...
// Description:
//    In this assignment we implement the RB Tree which is an
...
public class RBTreeDriver {
    public static void main (String args[]) {
       String line;
       BinarySearchTree <Book> rbTree = new RedBlackTree <Book> ();
       ...
       try {
           ...
           PrintWriter out = new PrintWriter(new FileWriter("RBTreeOutput.txt"));
           ...
           while (inFile.hasNextLine())  {
            ...
            inFile.close();
            out.println("All the " + rbTree.size() + " elements have been successfully " +
                             "inserted into the RB tree");
            out.println();
            out.println("The height of the tree is " + rbTree.height() );
            out.println();

            out.println("The minimum element in the tree is: ");
            out.println(rbTree.first() );

            out.println("\nThe maximum element in the tree is: ");
            out.println(rbTree.last() );
            out.println();
            out.println();

            // now start searching the RB tree tree
            while (testFile.hasNextLine()) {
                line = testFile.nextLine();
                Book aBook = new Book();
                aBook.setIsbn(line);
                searchCount.setData(0);
                Book result = rbTree.find(aBook, searchCount);
                if (result != null) {
